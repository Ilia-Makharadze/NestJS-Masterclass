    ...
    window.addEventListener('message', (event) => {
      if (event.data && event.data.type === 'compodoc-iframe-navigate') {
        ...
        sessionStorage.setItem(
          'compodocLastVisitedDocUrl',
          this.lastVisitedDocUrl,
        );
      }
    });
  ...
  setupResizer() {
    ...
    function handleMouseMove(e) {
      ...
      const containerRect = document
        .querySelector('.playground-content')
...
<p><strong>Type:</strong> ${templatePath}</p>`
  ...
  updateTemplateMetadata(templatePath, data) {
    ...
    document.getElementById('templateName').textContent =
      data.name || templatePath;
    document.getElementById('templateFile').textContent =
      data.file || `${templatePath}.hbs`;
  ...
  createVariableElements(
    ...
    const sortedEntries = Object.entries(obj).sort(([a], [b]) => {
      ...
      const aIsSimple =
        typeof obj[a] !== 'object' && typeof obj[a] !== 'function';
      const bIsSimple =
        typeof obj[b] !== 'object' && typeof obj[b] !== 'function';
      ...
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        ...
        if (depth < 3) {
          ...
          header.addEventListener('click', () => {
            ...
            toggle.style.transform = isCurrentlyExpanded
              ? 'rotate(0deg)'
              : 'rotate(90deg)';
          });
          ...
          const nestedContainer =
            objectElement.querySelector('.nested-variables');
          this.createVariableElements(
            value,
            fullKey,
            nestedContainer,
  ...
  async updateSessionConfig(configPath, newValue) {
    try {
      ...
      if (result.success) {
        this.showSuccess(
          '✅ Configuration saved! Documentation regenerating...',
        );
  ...
  async updatePreview() {
    ...
    try {
      this.setPreviewStatus(
        '🚀 Generating documentation with CompoDoc CLI...',
        true,
      );
      ...
      const response = await fetch(
        `/api/session/${this.sessionId}/generate-docs`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            templatePath: this.currentTemplate
      ...
      if (result.success) {
        ...
        if (iframe) {
          ...
          iframe.onerror = () => {
            this.setPreviewStatus(
              '❌ Failed to load generated documentation',
              false,
            );
          };
        } else {
    ...
    try {
      ...
      if (!response.ok) {
        if (
          ...
          const errorData = await response.json();
          throw new Error(
            errorData.error || 'Failed to create template package',
          );
        } else {
          throw new Error(
